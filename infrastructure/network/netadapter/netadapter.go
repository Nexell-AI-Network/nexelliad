...
import (
	...
	"github.com/Nexell-AI-Network/nexelliad/v2/infrastructure/network/netadapter/id"
	routerpkg "github.com/Nexell-AI-Network/nexelliad/v2/infrastructure/network/netadapter/router"
	"github.com/Nexell-AI-Network/nexelliad/v2/infrastructure/network/netadapter/server"
	"github.com/Nexell-AI-Network/nexelliad/v2/infrastructure/network/netadapter/server/grpcserver"
...
func (na *NetAdapter) onP2PConnectedHandler(connection server.Connection) error {
    // Obtenez l'adresse IP du pair
    tcpAddr := connection.Address()
    peerAddress := tcpAddr.IP.String() // Convertir l'adresse IP en chaîne

    // Vérifiez si l'adresse est autorisée
    if !isPeerAllowed(peerAddress) {
        log.Warnf("Rejected connection from untrusted peer: %s", peerAddress)
        return fmt.Errorf("connection rejected: untrusted peer")
    }
